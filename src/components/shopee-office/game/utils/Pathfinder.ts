/**
 * A* Pathfinder for Shopee Office scene.
 * Inspired by agent-town's Pathfinder with collision-aware grid navigation.
 */
...
export class Pathfinder {
  ...
  constructor(
    ...
    // Inflate collision rects by padding (half-body-width)
    ...
    for (let r = 0; r < this.rows; r++) {
      ...
      for (let c = 0; c < this.cols; c++) {
        ...
        for (const rect of inflated) {
          if (cellRight > rect.left && cellLeft < rect.right && cellBottom > rect.top && cellTop < rect.bottom) {
            walkable = false
            break
          }
        }
