/**
 * RPG-style Interaction Menu for agents.
 * Press E near an agent to open a contextual menu.
 * Inspired by agent-town's InteractionMenu class.
 */
...
export class InteractionMenu {
  ...
  show(worldX: number, worldY: number, options: MenuOption[]) {
    ...
    this.bg.fillRoundedRect(4, 4, MENU_WIDTH, totalH, 8)
    // Body
    this.bg.fillStyle(BG_COLOR, BG_ALPHA)
    this.bg.fillRoundedRect(0, 0, MENU_WIDTH, totalH, 8)
    // Top accent bar (Shopee brand)
    this.bg.fillStyle(ACCENT_COLOR, 1)
    this.bg.fillRoundedRect(0, 0, MENU_WIDTH, 4, { tl: 8, tr: 8, bl: 0, br: 0 })
    // Border
    this.bg.lineStyle(2, BORDER_COLOR, 1)
    this.bg.strokeRoundedRect(0, 0, MENU_WIDTH, totalH, 8)
